...
class HybridHawkesExpMixture(mpoints.hybrid_hawkes_exp.HybridHawkesExp):
    """
    This class implements state-dependent Hawkes processes with exponential mixture kernels, a subclass of hybrid marked point
    processes.
    ...
    :type number_of_kernels: int
    :param number_of_kernels: number of kernels in the mixture.
    """

    def __init__(self, number_of_event_types, number_of_states, events_labels, states_labels, number_of_kernels = 1):
        """
        ...
        :param number_of_kernels: number of kernels in the mixture.
        """
        super().__init__(number_of_event_types, number_of_states, events_labels, states_labels)
        self.number_of_kernels = number_of_kernels
        self.impact_coefficients = np.zeros(
            (number_of_event_types, number_of_states, number_of_event_types, number_of_kernels))
    ...
    def set_hawkes_parameters(self, base_rates, impact_coefficients, decay_coefficients):
        ...
        if np.shape(impact_coefficients) != (self.number_of_event_types, self.number_of_states,
                                             self.number_of_event_types, self.number_of_kernels):
    ...
    def kernel_at_time(time, alpha, beta):
        ...
        """
        return np.array(alpha * np.exp(- np.multiply(time, beta))).sum(axis=-1)
    ...
    def estimate_hawkes_parameters(self, times, events, states, time_start, time_end, maximum_number_of_iterations=2000,
                                   ...
                                   min_decay_coefficient=0.5, max_decay_coefficient=100):
        r"""
                    ...
                    for x in range(self.number_of_states):
                        for e2 in range(self.number_of_event_types):
                            for k in range(self.number_of_kernels):
                                u_min = math.log10(
                                    min_decay_coefficients[e1, x, e2, k])
                                u_max = math.log10(
                                    max_decay_coefficients[e1, x, e2, k])
                                u = np.random.uniform(u_min, u_max)
                                beta = 10 ** u
                                guess_decay_coefficients[e1, x, e2, k] = beta
                'Impact coefficients'
                    ...
                    for x in range(self.number_of_states):
                        for e2 in range(self.number_of_event_types):
                            for k in range(self.number_of_kernels):
                                ...
                                alpha = u * guess_decay_coefficients[e1, x, e2, k]
                                guess_impact_coefficients[e1, x, e2, k] = alpha
                'Save the random guess to the list of guesses'
                g = HybridHawkesExpMixture.parameters_to_array(guess_base_rates, guess_impact_coefficients,
        ...
        for g in guesses:
            dimension = self.number_of_event_types + 2 * \
                self.number_of_kernels * self.number_of_states * self.number_of_event_types ** 2
    ...
    def parameters_to_array(base_rates, impact_coefficients, decay_coefficients):
        ...
        number_of_kernels = s[3]
        result =\
            np.zeros(
                number_of_event_types_2
                + 2 * number_of_event_types_1
                * number_of_event_types_2 * number_of_states * number_of_kernels)
        ...
        for i in range(number_of_event_types_1):
            for j in range(number_of_states):
                for k in range(number_of_event_types_2):
                    for ke in range(number_of_kernels):
                        index = number_of_event_types_2 + ke * \
                            number_of_states * number_of_event_types_1 * number_of_event_types_2
                        index += j * number_of_event_types_1 * number_of_event_types_2
                        index += i * number_of_event_types_2 + k
                        result[index] = impact_coefficients[i, j, k, ke]
        for i in range(number_of_event_types_1):
            for j in range(number_of_states):
                for k in range(number_of_event_types_2):
                    for ke in range(number_of_kernels):
                        ...
                        index += ke * number_of_states * number_of_event_types_1 * number_of_event_types_2
                        index += j * number_of_event_types_1 * number_of_event_types_2
                        index += i * number_of_event_types_2 + k
                        result[index] = decay_coefficients[i, j, k, ke]
        return result
