...
\begin{equation*}
	\begin{align}
		...
		= & \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\frac{\partial}{\partial\theta_i}\lambda_i^{\theta_i}(t)}{\lambda^{\theta_1,\theta_2,\ldots}_\mathcal{D}(t)}d\Lambda_\mathcal{D} - \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\partial}{\partial\theta_i}\lambda_i^{\theta_i}(t)dt\\
		= & \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\frac{\partial}{\partial\theta_i}\lambda_i^{\theta_i}(t)}{\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)}B_{i,t}d\Lambda_\mathcal{D} - \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\partial}{\partial\theta_i}\lambda_i^{\theta_i}(t)dt,\\
		...
		= & \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\frac{\partial^2}{\partial\theta_i^2}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t) - \left(\frac{\partial}{\partial\theta_i}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)\right) \left(\frac{\partial}{\partial\theta_i}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)\right)^T }{\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)} B_{i,t} d\Lambda_\mathcal{D} \\
		  & - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \left(\frac{\partial^2}{\partial\theta^2}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)\right) dt,
	\end{align}
\end{equation*}
		%& - \left(\frac{\partial}{\partial\theta}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)\right) \left(\frac{\partial}{\partial\theta}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)\right)^T}{\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)}d\Lambda_\mathcal{D} - \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\partial^2}{\partial\theta^2}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t) dt \\
		%= & \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\frac{\partial^2}{\partial\theta^2}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t) \\
		%&- \left(\frac{\partial}{\partial\theta}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)\right)\left(\frac{\partial}{\partial\theta}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)\right)^T}{\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)}d\Lambda_\mathcal{D} - \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\partial^2}{\partial\theta^2}\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t) dt \\
where
$$B_{i,t} \coloneq \frac{\left(\lambda_i^{\theta_i}\right)_\mathcal{D}(t)}{\lambda_\mathcal{D}^{\theta_1,\theta_2,\ldots}(t)}$$
are the ratios of component intensities to overall intensity. In the case where the point process is a compound of processes with intensities $\lambda_i^{\theta_i}$, these represent the probability that an event at $t$ is from the $i$th point process.\textcolor{red}{Is this true? is there a simple proof?}

Suppose we have a Hawkes process with background intensity $\nu\in\mathbb{R}$ and kernel equal to
$$k(x) = \alpha\exp(-\beta x).$$

The loglikelihood is then equal to
\begin{equation*}
	\begin{align}
		\ell(\nu,\alpha,\beta;\mathcal{D})
		= &\int_{t_\mathrm{min}}^{t_\mathrm{max}}\log\left(\lambda_\mathrm{exo}(t)+\lambda_\mathrm{endo}(t)\right)d\Lambda_{\mathcal{D}}(t) \\
		- &\int_{t_\mathrm{min}}^{t_\mathrm{max}}\left(\lambda_\mathrm{exo}(t)+\lambda_\mathrm{endo}(t)\right) dt,
	\end{align}
\end{equation*}
where
\begin{equation*}
	\begin{align}
		\lambda_\mathrm{exo}(t) &\coloneq \nu,\\
		\lambda_\mathrm{endo}(t) &\coloneq \int_{t_\mathrm{min}}^t k(t-s) d\Lambda_{\mathcal{D}}(s) = \alpha\int_{t_\mathrm{min}}^t \exp(-\beta (t-s))d\Lambda_\mathcal{D}(s)
	\end{align}
\end{equation*}
are the exogenous and endogenous components of the intensity $\lambda_\mathcal{D}$.

We can then apply the chain rule to see that
\begin{equation*}
	\begin{align}
		\frac{\partial}{\partial \nu} \ell(\nu,\alpha,\beta;\mathcal{D})
		= &\int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\frac{\partial \lambda_\mathrm{exo}(t)}{\partial\nu}+\frac{\partial\lambda_\mathrm{endo}}{\partial\nu}}{\lambda_\mathrm{exo}(t)+\lambda_\mathrm{endo}(t)}d\Lambda_{\mathcal{D}}(t) - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \left(\frac{\partial \lambda_\mathrm{exo}(t)}{\partial\nu}+\frac{\partial\lambda_\mathrm{endo}}{\partial\nu}\right) dt \\
		= & \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\frac{\partial\mathrm{exo}}{\partial\nu}}{\lambda_\mathrm{exo}+\lambda_\mathrm{endo}}d\Lambda_\mathcal{D}(t) - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\partial \lambda_\mathrm{exo}(t)}{\partial\nu}dt\\
		= & \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\frac{\partial \lambda_\mathrm{exo}(t)}{\partial\nu}}{\lambda_\mathrm{exo}}B_{\mathrm{exo},t}d\Lambda_\mathcal{D}(t) - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\partial \lambda_\mathrm{exo}(t)}{\partial\nu}dt\\
		= & \frac{1}{\nu}\int_{t_\mathrm{min}}^{t_\mathrm{max}} B_{\mathrm{exo},t}d\Lambda_\mathcal{D}(t)-(t_\mathrm{max}-t_\mathrm{min}),\\
		\frac{\partial}{\partial \alpha} \ell(\nu,\alpha,\beta;\mathcal{D})
		= & \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\frac{\partial \lambda_\mathrm{endo}(t)}{\partial\alpha}}{\lambda_\mathrm{endo}}B_{\mathrm{endo},t}d\Lambda_\mathcal{D}(t) - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\partial \lambda_\mathrm{endo}(t)}{\partial\alpha}dt\\
		= & \frac{1}{\alpha}\left(\int_{t_\mathrm{min}}^{t_\mathrm{max}} B_{\mathrm{endo},t}d\Lambda_\mathcal{D}(t) - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \lambda_\mathrm{endo}(t)dt\right)\\
		\frac{\partial}{\partial \beta} \ell(\nu,\alpha,\beta;\mathcal{D})
		= & \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\frac{\partial \lambda_\mathrm{endo}(t)}{\partial\beta}}{\lambda_\mathrm{endo}}B_{\mathrm{endo},t}d\Lambda_\mathcal{D}(t) - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\partial \lambda_\mathrm{endo}(t)}{\partial\beta}dt,\\
	\end{align}
\end{equation*}
\textcolor{red}{Why can I exchange derivatives and integrals here}
where the second and third identities follow by similar reasoning to the first.

In general, if the intensity $\lambda$ can be written as a sum of stochastic processes $\lambda_i$, each drawn from a family of processes parameterised by $\theta_i$, and adapted to $\mathcal{F}$, the loglikelihood function of the resulting point process conditioning on a dataset $\mathcal{D}$ will satisfy
$$\frac{\partial}{\partial\theta_i}\ell(\theta_1,\theta_2,\ldots\theta_i,\ldots;\mathcal{D}) = \int_{t_\mathrm{min}}^{t_\mathrm{max}}\frac{\frac{\partial\lambda_i(t)}{\partial\theta_i}}{\lambda_i(t)}B_{i,t}d\Lambda_\mathcal{D}(t) - \int_{t_\mathrm{min}}^{t_\mathrm{max}} \frac{\partial\lambda_i(t)}{\partial\theta_i}dt$$
