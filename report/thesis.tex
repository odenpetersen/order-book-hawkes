...
Given a sample space $\Omega$ with probability measure $\mathbb P$, a point process is any increasing random sequence of times $$\mathcal T:\Omega\to T^\mathbb{N}.$$
\textcolor{red}{Include a barcode plot for a poisson process}
...
{\noindent}All \textcolor{red}{(or perhaps many)} of the point processes considered in this thesis will additionally be adapted with respect to some filtration $\mathcal F$.

In this thesis, we will only consider \textit{nonexplosive point processes}, defined as those point processes $\mathcal{T}$ for which
...
{\noindent}For any point process, there exists a corresponding stochastic process $N_{\mathcal T}:T\times\Omega\to\mathbb{N}$ known as the \textit{counting process}, that gives the number of events having occurred before or at a given time. This is defined as
$$N_{\mathcal T}(t,\omega) = \vert \{i\in\mathbb N \vert \mathcal{T}(\omega)_i \leq t\} \vert.$$
\textcolor{red}{this is a cadlag}
\textcolor{red}{Overlay barcode plot with corresponding counting process}
...
\textcolor{red}{Prove this is a measure}

Taking the expectation of the counting measure with respect to $\mathbb{P}$ \textcolor{red}{(optionally conditional on some sub-$\sigma$-algebra of $\Omega$)} gives the \textit{expectation measure} $\Lambda(S)$.
\textcolor{red}{Prove this is a measure}
