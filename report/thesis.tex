...
drawn from a set $X$, that represent additional information about each event.
...
Each part $E$ of the partition then has an associated point process $$\mathcal{T}_{\{n\in\mathbb{N} : \mathcal{X}_n\in E\}},$$ formed by the subsequence of times where the corresponding element of $\mathcal{X}$ is in $E$. These point processes will then have their own counting functions and intensities. It is common to arrange these in vector form, with one entry for each event type.
